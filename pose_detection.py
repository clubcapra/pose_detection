...
class PoseDectection:
    ...
    def detectPose(self, bodyData: Collection[float]) -> PoseType:
        ...
        vectors = getVectors(bodyData)

        angles = getAnglesFromVectors(vectors)
...
def getVectors(bodyData) -> list:
    ...
    for couple in KEYPOINT_COUPLES:
        vectors.append(
            getVectorFromPoints(
                bodyData[couple[0]], bodyData[couple[1]]
            )
        )
...
def getAnglesFromBodyData(bodyData: Collection[float]):
...
def getAnglesFromVectors(vectors):
